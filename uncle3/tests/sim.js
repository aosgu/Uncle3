...
(function () {
  ...
  // 轮询等待条件成立（应对仿真机性能波动），超时后返回最后一次条件结果
  ...
  async function run() {
    ...
    // ---------- T12 offscreen 10 分钟上限自动停止（缩短为 600ms 模拟） ----------
    sim.msgLog.length = 0;
    // 直接向 offscreen 派发 OFF_START（与真实链路一致；仿真中 background 也会收到并忽略）
    chrome.runtime.sendMessage({ type: 'OFF_START', streamId: 'limit-test', audio: false, maxMs: 600 });
