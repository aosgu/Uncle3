...
var dupLegacy = normalizePresets(undefined, [{ name: '旧HD', w: 1280, h: 720 }, { name: '自定义', w: 666, h: 666 }]);
eq('归一化 迁移按尺寸去重', dupLegacy.length, 5);
eq('归一化 去重保留内置 HD', dupLegacy.filter(function (p) { return presetKey(p) === '1280x720'; }).length, 1);
eq('归一化 保留非重复旧项', dupLegacy.some(function (p) { return p.name === '自定义'; }), true);
